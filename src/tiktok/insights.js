...
// Metrics مضمونة الصحة في كل حسابات TikTok
const TIKTOK_METRICS_SAFE = [
  'spend', 'impressions', 'clicks', 'ctr', 'cpm', 'frequency', 'reach',
  'conversion', 'cost_per_conversion', 'total_purchase_value'
];

// Metrics إضافية (تحتاج Pixel أو Video Ads)
const TIKTOK_METRICS_EXTENDED = [
  ...TIKTOK_METRICS_SAFE,
  'conversion_rate', 'purchase_roas',
  'add_to_cart', 'checkout',
  'video_watched_2s', 'video_watched_6s', 'video_play_complete_rate'
];

const TIKTOK_METRICS = TIKTOK_METRICS_EXTENDED; // نبدأ بـ extended، fallback للـ safe
...
async function getAdgroupInsightsToday(advertiserId) {
  ...
  try {
    const data = await tiktokGet('/report/integrated/get/', {
      ...
      report_type: 'BASIC',
      dimensions: JSON.stringify(['adgroup_id']),  // campaign_id incompatible with AUCTION_ADGROUP
...
async function getAdgroupInsightsFallback(advertiserId, today) {
  try {
    logger.warn(`[TikTok] Using safe metrics for ${advertiserId} (extended not available)`);
    const basicMetrics = TIKTOK_METRICS_SAFE;
    const data = await tiktokGet('/report/integrated/get/', {
      advertiser_id: advertiserId,
      report_type: 'BASIC',
      dimensions: JSON.stringify(['adgroup_id']),  // campaign_id incompatible with AUCTION_ADGROUP
