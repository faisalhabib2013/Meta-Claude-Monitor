...
function upsertDailyMetrics(date, adsetId, data) {
  ...
  mutate(db => {
    db.daily_metrics[key] = {
      ...
      adset_created_at: data.createdAt || null
    };
  });
}
...
// تاريخ adset معين خلال آخر N يوم (للـ rolling CPP)
function getAdsetHistory(adsetId, days = 3) {
  const db = getDb();
  const cutoff = new Date(Date.now() - (days - 1) * 86400000).toISOString().slice(0, 10);
  return Object.values(db.daily_metrics)
    .filter(r => r.adset_id === adsetId && r.date >= cutoff)
