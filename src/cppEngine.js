...
async function runMonitoringCycle(scaledThisCycle = new Set()) {
    ...
    try {
      ...
      for (const row of insights) {
        ...
        db.upsertDailyMetrics(today, adsetId, {
          ...
          thresholdExceeded: 0,
          createdAt: adset.created_time || null
        });

        // Rolling 3-day CPP + عمر الـ Ad Set
        const history = db.getAdsetHistory(adsetId, 3);
        const histSpend = history.reduce((s, h) => s + (h.spend || 0), 0);
        const histPurch = history.reduce((s, h) => s + (h.purchases || 0), 0);
        const rollingCpp = histPurch > 0 ? histSpend / histPurch : 0;
        const ageDays = adset.created_time
          ? Math.max(0, Math.floor((Date.now() - new Date(adset.created_time).getTime()) / 86400000))
          : null;
        const extras = { rollingCpp, rollingDays: 3, ageDays };
        ...
        if (isScaleOpportunity && !scaledThisCycle.has(product.name)) {
          ...
          if (!scaleAlerted) {
            ...
            scaledThisCycle.add(product.name);
...
async function runTiktokMonitoringCycle(scaledThisCycle = new Set()) {
    ...
    try {
      ...
      for (const row of insights) {
        ...
        db.upsertDailyMetrics(today, `tt_${adgroupId}`, {
          ...
          purchaseRoas: metrics.purchaseRoas,
          thresholdExceeded: 0,
          createdAt: adgroup.create_time ? adgroup.create_time.replace(' ', 'T') : null
        });

        processedCount++;

        // Rolling 3-day CPP + عمر الـ Ad Group
        const ttHistory = db.getAdsetHistory(`tt_${adgroupId}`, 3);
        const ttSpend = ttHistory.reduce((s, h) => s + (h.spend || 0), 0);
        const ttPurch = ttHistory.reduce((s, h) => s + (h.purchases || 0), 0);
        const ttRollingCpp = ttPurch > 0 ? ttSpend / ttPurch : 0;
        const ttAgeDays = adgroup.create_time
          ? Math.max(0, Math.floor((Date.now() - new Date(adgroup.create_time.replace(' ', 'T')).getTime()) / 86400000))
          : null;
        const ttExtras = { rollingCpp: ttRollingCpp, rollingDays: 3, ageDays: ttAgeDays };
        ...
        if (isTikTokScale && !scaledThisCycle.has(`tt_${product.name}`)) {
          ...
          if (!scaleAlerted) {
            ...
            scaledThisCycle.add(`tt_${product.name}`);
          }
        }
      }
