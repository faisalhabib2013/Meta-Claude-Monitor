...
// رفع الميزانية بنسبة معينة
async function increaseBudget(adsetId, percentIncrease) {
  const adsetData = await metaGet(`/${adsetId}`, {
    fields: 'name,daily_budget,lifetime_budget'
  });
  const currentBudgetPiastres = parseInt(adsetData.daily_budget || 0);
  if (!currentBudgetPiastres) {
    return { success: false, error: 'لا يوجد daily budget (CBO)' };
  }
  const pct = Math.max(1, Math.min(200, percentIncrease));
  const newBudgetPiastres = Math.round(currentBudgetPiastres * (1 + pct / 100));
  await metaPost(`/${adsetId}`, { daily_budget: newBudgetPiastres });
  logger.success(`Budget increased for ${adsetId}: ${currentBudgetPiastres/100} → ${newBudgetPiastres/100} EGP (+${pct}%)`);
  return {
    success: true,
    oldBudgetEGP: currentBudgetPiastres / 100,
    newBudgetEGP: newBudgetPiastres / 100,
    percentIncrease: pct,
    adsetName: adsetData.name
  };
}

module.exports = { pauseAdset, reduceBudget, increaseBudget, getAdsetBudget };
