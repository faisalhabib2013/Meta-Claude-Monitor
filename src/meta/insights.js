...
// توزيع المشتريات حسب ساعات اليوم (بتوقيت الحساب الإعلاني)
async function getHourlyPurchasesToday(accountId) {
  try {
    const data = await metaGetAll(`/act_${accountId}/insights`, {
    ...
    logger.error(`Hourly breakdown failed for ${accountId}`, err);
    return [];
  }
}
