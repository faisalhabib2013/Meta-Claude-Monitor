...
const { formatPauseConfirmation, formatBudgetConfirmation, fmt, escapeMd } = require('../utils/format');
...
async function handleCallback(query) {
  ...
  } else if (data.startsWith('scale_up:')) {
    await handleScaleUpRequest(chatId, messageId, data);
  } else if (data.startsWith('confirm_scale:')) {
    await handleConfirmScaleUp(chatId, messageId, data);
  } else if (data.startsWith('pause_product_confirm:')) {
    await handlePauseProductConfirm(chatId, messageId, data);
...
async function handlePauseRequest(chatId, messageId, data) {
  ...
  await editMessage(chatId, messageId,
    `⏸ *تأكيد إيقاف Ad Set*\n\n` +
    `📑 *${escapeMd(ctx.adsetName)}*\n` +
    `🗂 ${escapeMd(ctx.campaignName)}\n\n` +
...
async function handleConfirmPause(chatId, messageId, data) {
  ...
  try {
    ...
    if (ctx.platform === 'tiktok') {
      const { pauseTiktokAdgroup } = require('../tiktok/actions');
      result = await pauseTiktokAdgroup(ctx.accountId, ctx.adsetId.replace('tt_', ''));
    } else {
      result = await pauseAdset(ctx.adsetId);
    }
...
async function handleBudgetReduction(chatId, messageId, data) {
  ...
  try {
    let result;
    if (ctx.platform === 'tiktok') {
      const { reduceTiktokBudget } = require('../tiktok/actions');
      result = await reduceTiktokBudget(ctx.accountId, ctx.adsetId.replace('tt_', ''), percent);
    } else {
      result = await reduceBudget(ctx.adsetId, percent);
    }
...
// Scale Up — عرض التأكيد
async function handleScaleUpRequest(chatId, messageId, data) {
  const parts = data.split(':');
  const ctxKey = parts[1];
  const percent = parseInt(parts[2]);
  const ctx = getContext(ctxKey);
  ...
  const currentBudgetEGP = ctx.platform === 'tiktok'
    ? ctx.currentBudgetEGP
    : ctx.currentBudgetPiastres / 100;
  const newBudget = (currentBudgetEGP * (1 + percent / 100)).toFixed(2);

  const keyboard = {
    inline_keyboard: [[
      { text: `✅ تأكيد +${percent}%`, callback_data: `confirm_scale:${ctxKey}:${percent}` },
      { text: '❌ إلغاء', callback_data: 'cancel' }
    ]]
  };
  ...
  await editMessage(chatId, messageId,
    `🚀 *تأكيد رفع الميزانية*\n\n` +
    `📑 ${escapeMd(ctx.adsetName)}\n` +
    `💰 ${currentBudgetEGP} ج.م → *${newBudget} ج.م* (+${percent}%)`,
    { reply_markup: keyboard }
  );
}

// Scale Up — تنفيذ
async function handleConfirmScaleUp(chatId, messageId, data) {
  const parts = data.split(':');
  const ctxKey = parts[1];
  const percent = parseInt(parts[2]);
  const ctx = getContext(ctxKey);
  ...
  try {
    let result;
    if (ctx.platform === 'tiktok') {
      const { increaseTiktokBudget } = require('../tiktok/actions');
      result = await increaseTiktokBudget(ctx.accountId, ctx.adsetId.replace('tt_', ''), percent);
    } else {
      const { increaseBudget } = require('../meta/actions');
      result = await increaseBudget(ctx.adsetId, percent);
    }

    if (!result.success) {
      return editMessage(chatId, messageId, `❌ فشل رفع الميزانية\n${result.error}`);
    }

    const successMsg = `✅ *تم رفع الميزانية*\n\n📑 ${escapeMd(result.adsetName)}\n💰 ${result.oldBudgetEGP.toFixed(2)} ج.م → *${result.newBudgetEGP.toFixed(2)} ج.م* (+${percent}%)`;
    await editMessage(chatId, messageId, successMsg);
    ...
    for (const cid of config.telegram.chatIds) {
      if (String(cid) !== chatId) {
        const { sendTo: st } = require('./bot');
        await st(cid, `🚀 *تم رفع ميزانية Scale*\n📑 ${escapeMd(result.adsetName)}\n+${percent}% → ${result.newBudgetEGP.toFixed(2)} ج.م\n👤 بواسطة: ${chatId}`);
      }
    }
  } catch (err) {
    await editMessage(chatId, messageId, `❌ خطأ: ${err.message}`);
  }
}

// Pause Product — تنفيذ إيقاف كل ad sets لمنتج
async function handlePauseProductConfirm(chatId, messageId, data) {
  const ctxKey = data.replace('pause_product_confirm:', '');
  const ctx = getContext(ctxKey);
  if (!ctx) return sendTo(chatId, '❌ انتهت صلاحية هذا الطلب.');

  const { pauseAdset } = require('../meta/actions');
  const { pauseTiktokAdgroup } = require('../tiktok/actions');
  const results = { paused: [], failed: [] };

  await editMessage(chatId, messageId, `⏳ جاري إيقاف ${Object.keys(ctx.adsets).length} Ad Sets...`);

  for (const [adsetId, metric] of Object.entries(ctx.adsets)) {
    try {
      if (adsetId.startsWith('tt_')) {
        await pauseTiktokAdgroup(metric.account_id, adsetId.replace('tt_', ''));
      } else {
        await pauseAdset(adsetId);
      }
      results.paused.push(metric.adset_name);
    } catch (err) {
      results.failed.push(metric.adset_name);
    }
    await new Promise(r => setTimeout(r, 500));
  }

  let summary = `✅ *تم إيقاف منتج: ${ctx.pauseProductName}*\n\n`;
  if (results.paused.length) {
    summary += `✅ تم إيقافهم (${results.paused.length}):\n`;
    results.paused.forEach(n => { summary += `• ${escapeMd(n)}\n`; });
  }
  if (results.failed.length) {
    summary += `\n❌ فشل إيقافهم (${results.failed.length}):\n`;
    results.failed.forEach(n => { summary += `• ${escapeMd(n)}\n`; });
  }
