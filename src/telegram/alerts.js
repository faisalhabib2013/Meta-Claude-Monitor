...
async function sendCppAlert(alertData) {
  const { product, campaign, adset, metrics, alertInfo, accountId } = alertData;
...
async function sendStatusMessage(chatId) {
  ...
  if (todayMetrics.length > 0) {
    ...
    for (const row of todayMetrics) {
      ...
      text += `${status} ${row.product_name}: CPP ${cpp.toFixed(0)} ج.م | Spend ${row.total_spend.toFixed(0)} ج.م\n`;
    }
  } else {
