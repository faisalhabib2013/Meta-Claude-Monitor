...
async function broadcastLong(text, options = {}) {
  const MAX_LEN = 3800; // buffer دون الحد

  if (text.length <= MAX_LEN) {
    return [await broadcast(text, options)];
  }

  // تقسيم عند السطور لتجنب كسر الكلمات
  const chunks = [];
  let current = '';
  ...
  for (let i = 0; i < chunks.length; i++) {
    const header = chunks.length > 1 ? `_(${i + 1}/${chunks.length})_\n` : '';
    await broadcast(header + chunks[i], options);
